...
function findById(id, managerId, userId) {
  return new Promise((resolve, reject) => {
    con.query(
      'SELECT * FROM competitions WHERE id = ? AND manager = ? AND userId = ?',
      [id, managerId, userId],
      function (err, result) {
        if (err) reject(err)
        else {
...
function findByName(name, userId) {
  return new Promise((resolve, reject) => {
    con.query(
      'SELECT * FROM competitions WHERE name = ? AND userId = ?',
      [name, userId],
...
function findByManager(id, userId) {
  return new Promise((resolve, reject) => {
    con.query(
      'SELECT * FROM competitions WHERE manager = ? AND userId = ?',
      [id, userId],
      function (err, competition) {
        if (err) reject(err)
        else resolve(competition)
...
function saveCompetition(competition) {
  return new Promise((resolve, reject) => {
    con.query('INSERT INTO competitions SET ?', competition, function (
      err,
      result
    ) {
      if (err) reject(err)
...
function updateCompetition(competition, id, userId) {
  return new Promise((resolve, reject) => {
    con.query(
      'UPDATE competitions SET ? WHERE id = ? AND userId = ?',
...
function deleteCompetition(id, userId) {
  return new Promise((resolve, reject) => {
    con.query(
      'DELETE FROM competitions WHERE id = ? AND userId = ?',
      [id, userId],
      function (err, result) {
        if (err) reject(err)
        else resolve(result)
