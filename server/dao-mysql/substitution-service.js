...
function findById(id, userId) {
  return new Promise((resolve, reject) => {
    con.query(
      'SELECT * FROM substitutions WHERE id = ? AND userId = ?',
      [id, userId],
      function (err, substitution) {
        if (err) reject(err)
...
function saveSubstitution(substitution) {
  return new Promise((resolve, reject) => {
    con.query('INSERT INTO substitutions SET ?', substitution, function (
      err,
      result
    ) {
      if (err) reject(err)
...
function deleteSubstitution(id) {
  return new Promise((resolve, reject) => {
    con.query('DELETE FROM substitutions WHERE id = ?', id, function (
      err,
      result
    ) {
      if (err) reject(err)
