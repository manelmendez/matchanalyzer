  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      for (let c = 0; c < competitions.length; c++) {
        let rounds = roundService.findByCompetition(competitions[c].id, userId) //talvez hay que poner await en estas 3 y async en la new promise
        let matches = matchService.findByCompetition(competitions[c].id, userId)
        let teams = teamService.findByCompetition(competitions[c].id, userId)
