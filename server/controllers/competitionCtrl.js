export class CompetitionController {
  constructor ({ CompetitionModel, TeamModel, RoundModel, MatchModel }) {
  ...
  addCompetition = async (req, res) => {
    ...
    try {
      const competitionSaved =
        await this.competitionModel.saveCompetition({ competition })
  ...
  getCompetition = async (req, res) => {
    ...
    try {
      const result = await this.competitionModel.findById({ id, managerId, userId })
      if (result.length !== 0) {
        const competition = JSON.parse(JSON.stringify(result))
        const teams = await this.teamModel.findByCompetition({ competitionId: id, userId })
  ...
  getUserCompetitions = async (req, res) => {
    ...
    try {
      const competitions = await this.competitionModel.findByManager({ id, userId })
  ...
  deleteCompetition = async (req, res) => {
    ...
    const id = req.params.id
    try {
      await this.competitionModel.deleteCompetition({ id, userId })
  ...
  getCompetitionRanking = async (competitions, userId) => {
    const competitionsWithStats = []
    for (let c = 0; c < competitions.length; c++) {
      const rounds = await this.roundModel.findByCompetition({ competitionId: competitions[c].id, userId })
      const matches = await this.matchModel.findByCompetition({ competitionId: competitions[c].id, userId })
      const teams = await this.teamModel.findByCompetition({ competitionId: competitions[c].id, userId })
