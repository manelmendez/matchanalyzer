...
export class MatchController {
  ...
  getMatch = async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.user) {
  ...
  addMatch = async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.user) {
        return errorHelper.unauthorizedError('No userId provided in Auth')
      }
  ...
  deleteMatch = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const id: number = Number(req.params.id)
      if (!req.user) {
        return errorHelper.unauthorizedError('No userId provided in Auth')
