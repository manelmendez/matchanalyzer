...
import * as competitionService from '../dao/competition-service.ts'
...
function addCompetition(req: any, res: any) {
...
function getCompetition(req: any, res: any) {  
  let id = req.params.id;
  let userId = req.user.id;
  console.log("Buscando competición con id: " + id + " en la base de datos...");
  competitionService.findById(id, req.user.id, userId).then(((result: any) => {    
    if (result.length != 0) {
...
async function getUserCompetitions(req: any, res: any) {
  let id = req.params.id;
  let userId = req.user.id;
  ...
  try {
    ...
    let competitionsWithStats = await getCompetitionRanking(competitions, userId);
...
function updateCompetition(req: any, res: any) {
  let userId = req.user.id;
  const competition = {
    name: req.body.name,
    season: req.body.season,
  ...
  competitionService.updateCompetition(competition, req.params.id, userId).then(() => {    
...
function deleteCompetition(req: any, res: any) {
  let userId = req.user.id;
...
async function getCompetitionRanking(competitions: any, userId: any) {
  let competitionsWithStats = [];
  for (let c = 0; c < competitions.length; c++) {
    let rounds: any = await roundService.findByCompetition(competitions[c].id, userId);
    let matches: any = await matchService.findByCompetition(competitions[c].id, userId);
    let teams: any = await teamService.findByCompetition(competitions[c].id, userId);
