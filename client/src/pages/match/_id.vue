...
export default {
  ...
  methods: {
    ...
    async createMatchpart(data) {
      let matchpart = {
        ...
        team: this.match.localTeam.manager ? this.match.localTeam.id : this.match.awayTeam.id
      }
  ...
  computed: {
    ...
    matchparts() {
      return this.$store.getters['competition/matchpartsByMatch'](this.$route.params.matchId)
    },
    minutes() {
      return this.$store.getters['competition/minutesByMatch'](this.$route.params.matchId)
    },
    goals() {
      return this.$store.getters['competition/goalsByMatch'](this.$route.params.matchId)
    },
    assists() {
      return this.$store.getters['competition/assistsByMatch'](this.$route.params.matchId)
    },
    cards() {
      return this.$store.getters['competition/cardsByMatch'](this.$route.params.matchId)
    },
    substitutions() {
      return this.$store.getters['competition/substitutionsByMatch'](this.$route.params.matchId)
    },
  },
  watch: {
    async match() {
      ...
      this.match.localTeam.players = this.$store.getters['team/playersByTeamId'](this.match.localTeam.id)
      await this.getTeam(this.match.awayTeam)
      this.match.awayTeam = this.$store.getters['team/teamById'](this.match.awayTeam)
      await this.getPlayersByTeamId(this.match.awayTeam.id)
      this.match.awayTeam.players = this.$store.getters['team/playersByTeamId'](this.match.awayTeam.id)
      this.putData()
    },
  }
}
